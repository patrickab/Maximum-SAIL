...
def run_custom_sail(self: SailRun):
    ...
    while(current_eval_budget >= BATCH_SIZE):
        ...
        candidate_solutions_df = select_samples(self, improved_elites=improved_elites, new_bin_elites=new_bin_elites, acq_flag=True)          # Select samples based on exploration behavior defined in the class constructor
        solution_batch = candidate_solutions_df.solution_batch()
        measures_batch = candidate_solutions_df.measures_batch()
        obj_t0, obj_t1 = eval_xfoil_loop(self, solution_batch=solution_batch, measures_batch=measures_batch, acq_flag=True)                   # Evaluate Acquisition Elites & Update Acq Archive under resulting GP Model
...
def select_samples(self: SailRun, improved_elites, new_bin_elites, acq_flag=False, pred_flag=False):
    ...
    if self.hybrid_flag: # Evenly balance sampling of best new_bin_elites & best improved_elites
        ...
        if n_new_bin_elites >= n_samples//2 and n_improved_elites >= n_samples//2:
            candidate_elite_df = pandas.concat([new_bin_elites.head(n_samples//2), improved_elites.head(n_samples//2)])
        else:
...
def prediction_verification_loop(self: SailRun):
    ...
    while(current_eval_budget >= iter_evals):
        ...
        test_pred_t0 = self.pred_archive.stats.num_elites
        ...
        test_pred_t1 = self.pred_archive.stats.num_elites

        if pred_t0 != test_pred_t0 or pred_t1 != test_pred_t1:
            raise ValueError("Acq Archive Size Mismatch")
        ...
        solution_batch = candidate_solutions_df.solution_batch()
        measures_batch = candidate_solutions_df.measures_batch()
        obj_t0, obj_t1 = eval_xfoil_loop(self, solution_batch=solution_batch, measures_batch=measures_batch, pred_flag=True)                              # Evaluate Acquisition Elites & Update Acq Archive under resulting GP Model
