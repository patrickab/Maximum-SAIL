...
from xfoil.simulate_airfoils_multiprocess import xfoil
from xfoil.generate_airfoils_multiprocess import generate_parsec_coordinates
...
def remove_worst_elites(archive, n_elites):
    """
    Removes the worst elites from the archive
    """
    sorted_elites = sorted(archive, key=lambda x: x.objective, reverse=True)
    worst_elites = sorted_elites[n_elites:]
    for elite in worst_elites:
        archive.remove(elite.solution)
...
def sail(initial_seed):
    ...
    obj_archive, init_solutions, init_obj_evals = initialize_archive(obj_archive, seed, initial_seed)
    ...
    while(eval_budget >= BATCH_SIZE):
        ...
        # select the best elites & update their acquisition values
        best_elites = sorted(acq_archive, key=lambda x: x.objective, reverse=True)[:BATCH_SIZE*3]
        best_elite_solutions = np.array([elite.solution for elite in best_elites])
        best_elite_acquisitions = acq_ucb(best_elite_solutions, gp_model)

        # clear acq_archive & add updated best elites 
        acq_archive.clear()
        acq_archive.add([elite.solution for elite in best_elites], best_elite_acquisitions.squeeze(), [elite.measures for elite in best_elites])

        acq_archive = map_elites(acq_archive, acq_emitter, gp_model, ACQ_N_MAP_EVALS, acq_ucb)

        # remove the worst 20 acq_elites
        best_elites = best_elites = sorted(acq_archive, key=lambda x: x.objective, reverse=True)[:acq_archive.stats.num_elites-20]
        acq_archive.add([elite.solution for elite in best_elites], [elite.objective for elite in best_elites], [elite.measures for elite in best_elites])
        ...
        convergence_errors, success_indices, obj_batch = xfoil(iterations=BATCH_SIZE, initial_seed=initial_seed)
    ...
    pred_archive = map_elites(pred_archive, obj_emitter, gp_model, PRED_N_EVALS, predict_objective)
...
def verify_prediction_archive(pred_dataframe, initial_seed):
        ...
        for i in range(0, elites.shape[0], BATCH_SIZE):
            ...
            convergence_errors, success_indices, true_obj_batch = xfoil(iterations=elite_batch.shape[0], initial_seed=initial_seed)
...
def sail_iteration(i):

    global mse_array, qd_score_array, data
    ...
    verified_obj_archive, unverified_obj_archive, pred_error_archive = verify_prediction_archive(pred_dataframe, i)
    ...
    print("MSE: " + str(mse) + "\nQD-Score: " + str(qd_score) + "\n\n")

    # copy files to windows directory for usage in Rstudio
    subprocess.run(f"cp pred_archive_{i}.csv verified_obj_archive_{i}.csv pred_error_archive_{i}.csv unverified_obj_archive_{i}.csv /mnt/c/Users/patri/Desktop/Thesis/archives", shell=True, check=True)
    ...
    with open("run_{i}_data.json", "w") as json_file:
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=n_cores) as executor:
        for i in range(TEST_RUNS):
            executor.submit(sail_iteration, i)

    end_time = time.time()
    print("\nTotal Runtime: " + str(round((end_time - start_time) / 60, 2)) + " minutes")